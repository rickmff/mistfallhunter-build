  ...
  <v-row density="comfortable">
    <v-col cols="6">
      <v-card variant="tonal" color="primary" class="pa-3 h-100">
        ...
        <div v-if="plan.presetSavings" class="text-body-small text-success font-weight-medium">
          presets já poupam {{ plan.presetSavings }} g
        </div>
      </v-card>
    </v-col>
    ...
    <v-col cols="6">
      <v-card class="pa-3 h-100">
        ...
        <div class="text-body-small text-disabled">
          {{ plan.boughtPieces.length }} itens<template v-if="plan.unpricedUsed">
            · +{{ plan.unpricedUsed }} s/ preço</template
          >
        </div>
      </v-card>
    </v-col>
